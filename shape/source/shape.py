...
'''
Inheritance models is called is a relationship. In the current context Circle class is 
specialized version of Base(Shape) class.
'''
...
class Circle(Shape):
    ...
    def __init__(self, radius: float) -> None:
        ...
        if self.radius < 0:
            raise ValueError("Cannot compute perimeter and area of negative radius number")
        self.radius = radius
...
if __name__ == "__main__":
    c1 = Circle(radius=4)
    print(f"The first object circle create is: {c1}")
    perimeter = c1.perimeter()
    area = c1.area()
    print(f"The perimeter of the circle provided is perimeter = {perimeter} units")
    print(f"The area of the circle provide is: area= {area} square units")
